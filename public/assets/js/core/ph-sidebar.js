...
const PHSidebar = (() => {
  ...
  const ADMIN_NAV = [
    { group: 'Admin',   href: 'pages/admin/dashboard.html',          key: 'admin',        icon: `<rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/>`, fill: 'currentColor', label: 'Admin Panel' },
  ];
  ...
  function render(active, base = '../../', isAdmin = false) {
  ...
  function init(active, base = '../../') {
    ...
    const doRender = () => {
      // Determine if user is admin from JWT (skip in dev mode)
      let isAdmin = IS_DEV; // in dev mode, show all nav including admin
      if (!IS_DEV) {
        try {
          const tk = localStorage.getItem('access_token');
          if (tk) {
            const p = JSON.parse(atob(tk.split('.')[1]));
            isAdmin = p.role === 'admin' || p.isAdmin === true;
          }
        } catch(e) {}
      }

      render(active, base, isAdmin);
      buildOverlay();
