...
export function HiveAIWorkspace({
  ...
  return (
    <div className={cn(
      ...
      {/* ── Workspace Body (Left Sidebar + Center Canvas + Right Split-Pane) ── */}
      <div className="flex-1 flex min-h-0 overflow-hidden relative">
        {/* ── Left Studio Sidebar (Categorized Intelligence Modes) ───────── */}
        {sidebarOpen && (
          <aside className="hidden md:flex flex-col w-64 lg:w-72 border-r border-border/70 bg-card/40 shrink-0 p-3.5 overflow-y-auto scrollbar-thin select-none">
            <div className="mb-2 px-1 flex items-center justify-between">
              <span className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground/80">Studio Modes</span>
              <span className="text-[10px] font-mono px-2 py-0.5 rounded-md bg-muted text-muted-foreground font-semibold">11 Modes</span>
            </div>
            <HiveAICapabilities
              variant="sidebar"
              activeCapability={capability}
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden">
          {/* Mobile Mode Switcher (only visible on mobile screens) */}
          <div className="md:hidden border-b border-border/70 bg-card/40 px-3 py-1">
            <HiveAICapabilities
              variant="pills"
              activeCapability={capability}
              onSelectCapability={(cap) => {
