...
export function HiveAIComposer({
  ...
  const currentCap = CAPABILITY_ITEMS.find((c) => c.id === activeCapability) || CAPABILITY_ITEMS[0];
  ...
  return (
    <>
      <div className={cn('p-3 sm:p-4 bg-transparent shrink-0 max-w-3xl mx-auto w-full', className)}>
        ...
        <div className="surface-floating rounded-2xl sm:rounded-3xl border border-white/10 shadow-2xl p-2.5 sm:p-3.5 space-y-2 transition-all focus-within:border-primary/50 focus-within:ring-1 focus-within:ring-primary/20 backdrop-blur-2xl">
          ...
          <div className="flex items-center justify-between gap-2 pt-1.5 border-t border-white/5">
            {/* Left: Mode Badge Selector, Attachments & Voice Dictation */}
            <div className="flex items-center gap-1.5 flex-wrap">
              {/* Active Mode Pill Selector */}
              {onSelectCapability && (
                <button
                  type="button"
                  onClick={() => setModalOpen(true)}
                  className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-muted/70 hover:bg-muted text-muted-foreground hover:text-foreground border border-border/60 text-xs font-semibold transition-all tap-press cursor-pointer shrink-0"
                  title="Switch AI Mode"
                >
                  <CapIcon className="w-3.5 h-3.5 text-primary" />
                  <span className="truncate max-w-[100px] sm:max-w-[140px]">{currentCap.shortLabel}</span>
      ...
      {/* Mode Picker Modal from Composer */}
      {onSelectCapability && (
